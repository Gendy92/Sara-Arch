-- Security Advisor hardening patch — addresses the most actionable WARN findings
-- after deploying v301. Run this in the Supabase SQL Editor as a single script.
--
-- What it does:
...
--
-- Warnings that remain after this patch (accepted/expected):
--   - authenticated users can execute SECURITY DEFINER RPCs such as dashboard_* and
--     admin_* — each function performs its own authorization check (is_app_admin).
--   - pg_net is installed in the public schema — required by the current Resend
--     implementation and cannot be moved without a larger refactor.
--   - schema_migrations and app_error_throttle have RLS enabled but no policies —
--     this is intentional; the tables are internal-only.
