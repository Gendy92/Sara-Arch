const Crud = {
  ...
  async addProcurement(vendorId) {
    ...
    const fields = [
      ...
      { name: 'client_id', label: 'العميل', type: 'select', section: 'التعريف', opts: [{ v: '', l: '-- اختر عميل --' }, ...clientOpts] },
    ...
    const modalBody = `<form>${UI.form(fields, { vendor_id: vendorId || '', date: new Date().toISOString().slice(0, 10) })}</form><div style="padding:12px 0;font-weight:600;color:var(--gold)">الإجمالي: <span id="proc-total">0.00 ج.م</span></div>`;
    const overlay = UI.openModal('إضافة مشتريات', modalBody, async (form) => {
      const fd = new FormData(form);
      const client = clients.find(c => c.id === fd.get('client_id'));
      ...
      const total = qty * up;
      // Default new procurements to immediate/cash. total_price is generated by the DB.
