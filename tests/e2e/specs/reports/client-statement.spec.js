...
test.describe('reports', () => {
  const prefix = `e2e-cstmt-${Date.now()}`;
  ...
  test('client statement shows deposits and expenses', async ({ page, api }) => {
    ...
    await projectsPage.addDeposit([
      { client_id: client.id, project_id: project.id, amount: '80000', payment_method: 'cash', date: today }
    ]);

    await clientsPage.goto();
    await clientsPage.openClient(`${prefix}-عميل`);
    await page.locator('button:has-text("كشف حساب")').first().click();
