...
test.describe('retention', () => {
  const prefix = `e2e-close-${Date.now()}`;
  ...
  test('closing project period creates supervision transaction', async ({ page, api }) => {
    ...
    await projectsPage.addExpense([
      { client_id: client.id, project_id: project.id, section_id: sectionId, amount: '50000', payment_method: 'cash', date: today }
    ]);

    await projectsPage.openProject(`${prefix}-مشروع`);
    await projectsPage.closePeriod(today);

    await expect(page.locator('text=تم قفل الدورة وإنشاء سجل الإشراف')).toBeVisible();
  });
});
