...
test.describe('projects', () => {
  ...
  test('records a project expense', async ({ page, api }) => {
    ...
    await projectsPage.addProject(`${prefix}-عميل`, [
      { name: `${prefix}-مشروع`, value: '400000', status: 'active' }
    ]);
    ...
    await projectsPage.addExpense([
      {
        ...
        payment_method: 'cash',
        date: today,
        description: 'مصروف تنفيذ'
      }
    ]);

    await expect(page.locator('text=تم حفظ 1 مصروف')).toBeVisible();
  });
});
