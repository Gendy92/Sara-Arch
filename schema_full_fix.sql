...
CREATE TABLE IF NOT EXISTS projects (
  ...
  retention_percentage NUMERIC DEFAULT 0 CHECK (retention_percentage BETWEEN 0 AND 100),
...
CREATE TABLE IF NOT EXISTS custody_records (
  ...
  returned_cash_amount NUMERIC DEFAULT 0,
  status TEXT DEFAULT 'active' CHECK (status IN ('active','settled','partial')),
  date DATE DEFAULT CURRENT_DATE,
  notes TEXT,
  advance_transaction_id UUID REFERENCES transactions(id),
  created_at TIMESTAMPTZ DEFAULT NOW(),
  updated_at TIMESTAMPTZ DEFAULT NOW(),
  deleted_at TIMESTAMPTZ
);
...
ALTER TABLE custody_records ADD COLUMN IF NOT EXISTS returned_cash_amount NUMERIC DEFAULT 0;
...
ALTER TABLE custody_records ADD COLUMN IF NOT EXISTS advance_transaction_id UUID REFERENCES transactions(id);
...
ALTER TABLE transactions DROP CONSTRAINT IF EXISTS transactions_type_check;
ALTER TABLE transactions ADD CONSTRAINT transactions_type_check CHECK (type IN ('project_deposit','project_expense','office_expense','owner_deposit','income','expense','deposit','withdrawal','supervision','client_return','vendor_settlement','custody_return','transfer','retention_withheld','retention_released'));
...
-- ┌─────────────────────────────────────────────────────────┐
-- │ STEP 5c: Custody state enforcement                      │
-- └─────────────────────────────────────────────────────────┘

ALTER TABLE custody_records ADD COLUMN IF NOT EXISTS remaining_balance NUMERIC DEFAULT 0;
...
-- Backfill remaining_balance, returned_amount and status for existing records
DO $$
DECLARE
  rec RECORD;
BEGIN
...
-- Admin password reset (does not rely on external email delivery).
...
BEGIN
  FOR rec IN
    SELECT cr.id, cr.amount, cr.employee_id, cr.employee_name, cr.date
    FROM custody_records cr
    WHERE cr.deleted_at IS NULL
      AND cr.advance_transaction_id IS NULL
  LOOP
    SELECT COALESCE(SUM(amount), 0) INTO v_spent
    FROM custody_expenses
    WHERE custody_id = rec.id AND deleted_at IS NULL;
    v_remaining := GREATEST((rec.amount - v_spent), 0);

    IF v_remaining > 0 THEN
      INSERT INTO transactions (type, amount, date, description, employee_id, employee_name, sector_name)
      VALUES (
        'office_expense',
        v_remaining,
        COALESCE(rec.date, CURRENT_DATE),
    ...
    UPDATE custody_records SET advance_transaction_id = v_tx_id WHERE id = rec.id;
  END LOOP;
END $$;
...
-- ┌─────────────────────────────────────────────────────────┐
-- │ v294: retention + supervision audit row functions       │
-- └─────────────────────────────────────────────────────────┘
