const Crud = {
  ...
  async projectStatement(projectId) {
    ...
    // Client (customer) summary — aggregated across ALL of the client's projects
    ...
    if (clientId) {
      ...
      clientSummaryHtml = `
        <h4 style="margin:0 0 8px">ملخص العميل: ${App.esc(clientName)}</h4>
        <div style="display:flex;gap:12px;flex-wrap:wrap;margin-bottom:16px">
          <div class="kpi-card" style="flex:1;min-width:140px"><div class="kpi-label">إجمالي إيداعات العميل (كل المشاريع)</div><div class="kpi-value" style="color:var(--green)">${App.fmtMoney(clientTotals.dep)}</div></div>
          <div class="kpi-card" style="flex:1;min-width:140px"><div class="kpi-label">إجمالي مصروفات العميل (كل المشاريع)</div><div class="kpi-value" style="color:var(--red)">${App.fmtMoney(clientTotals.exp)}</div></div>
    ...
    const summary = `<h4 style="margin:0 0 8px">كشف حساب المشروع: ${App.esc(name)}</h4>
    <div style="display:flex;gap:12px;flex-wrap:wrap;margin-bottom:16px">
      <div class="kpi-card" style="flex:1;min-width:140px"><div class="kpi-label">إجمالي الإيداعات</div><div class="kpi-value" style="color:var(--green)">${App.fmtMoney(totalDep)}</div></div>
      <div class="kpi-card" style="flex:1;min-width:140px"><div class="kpi-label">إجمالي المصروفات</div><div class="kpi-value" style="color:var(--red)">${App.fmtMoney(totalExp)}</div></div>
      <div class="kpi-card" style="flex:1;min-width:140px"><div class="kpi-label">الرصيد</div><div class="kpi-value">${App.fmtMoney(totalDep - totalExp)}</div></div>
    ...
    UI.openModal(`كشف حساب مشروع: ${App.esc(name)}`, downloadBtn + clientSummaryHtml + summary + table, null);
  },
  ...
  async projectBudget(projectId) {
    const [project, txs, procs] = await Promise.all([
      ...
      API.fetchAll('transactions', `?select=*&project_id=eq.${projectId}&deleted_at=is.null`),
      API.fetchAll('procurements', `?select=total_price&project_id=eq.${projectId}&deleted_at=is.null`)
    ]);
    const p = project[0];
  ...
  async vendorStatement(vendorId) {
    const [vendor, txs, procs] = await Promise.all([
      ...
      API.fetchAll('transactions', `?select=*,projects(name)&vendor_id=eq.${vendorId}&type=eq.project_expense&deleted_at=is.null&order=date.desc`),
